...
/**
 * Paginated result for resource queries
 */
export interface IResourcePageResult {
...
export const useResourceSchemaExtension = ({
  ...
  > => {
    ...
    try {
      ...
      if (storedIds.length === 0) {
        logInfo(
          "getSchemaExtensions",
          "No schema extension IDs found in storage",
        );
        return [];
      }
      ...
      logInfo(
        "getSchemaExtensions",
        `Found ${storedIds.length} schema extension IDs in storage`,
      );
      ...
      logInfo(
        "getSchemaExtensions",
        `Retrieved ${schemaExtensions.length} schema extensions from Graph`,
      );
  ...
  const getSchemaExtension = React.useCallback(
    async (id: string): Promise<ISchemaExtension | undefined> => {
      ...
      try {
        ...
        const handledError = handleGraphError(err, "getSchemaExtension", {
          schemaId: id,
        });
        setError(handledError.message);
        throw handledError;
  ...
  /**
   * Get resources by target type with optional search and pagination
   */
  const getResourcesByType = React.useCallback(
    async (
      ...
      try {
        ...
        // If we have a pageToken (nextLink), use it directly
        if (pageToken) {
          const response = await client.api(pageToken).get();
          const nextLink = response["@odata.nextLink"] as string | undefined;

          logInfo(
            "getResourcesByType",
            `Retrieved ${response.value?.length || 0} ${targetType} resources (next page)`,
          );
        ...
        const displayProperty =
          DISPLAY_PROPERTY_MAP[targetType] || "displayName";

        let apiRequest = client.api(endpoint).top(pageSize);

        // Add search filter if provided
        if (searchQuery?.trim()) {
          // Use $search for users and groups (more powerful search)
          if (
            targetType === ETargetTypes.User ||
            targetType === ETargetTypes.Group
          ) {
            ...
            apiRequest = apiRequest.filter(
              `startswith(${displayProperty},'${searchQuery}')`,
            );
          }
        }
        ...
        logInfo(
          "getResourcesByType",
          `Retrieved ${response.value?.length || 0} ${targetType} resources`,
        );
        ...
        const handledError = handleGraphError(err, "getResourcesByType", {
          targetType,
          searchQuery,
        });
        setError(handledError.message);
  ...
  const getResourceAttributes = React.useCallback(
    async (
      ...
      if (
        !targetType?.trim() ||
        !resourceId?.trim() ||
        !schemaExtensionId?.trim()
      ) {
        throw new Error(
          "Target type, resource ID, and schema extension ID are required",
        );
      }
      ...
      try {
        ...
        logInfo(
          "getResourceAttributes",
          `Response for ${targetType}/${resourceId}`,
        ...
        if (extensionData) {
          logInfo(
            "getResourceAttributes",
            `Found existing attributes for ${schemaExtensionId}`,
            { extensionData: JSON.stringify(extensionData) },
          );
        } else {
          logInfo(
            "getResourceAttributes",
            `No existing attributes found for ${schemaExtensionId} on ${targetType}/${resourceId}`,
          );
        }
        ...
        if (
          ...
          logInfo(
            "getResourceAttributes",
            `Resource not found: ${targetType}/${resourceId}`,
          );
          return {};
        }
        ...
        if (graphError.message?.includes("Could not find a property named")) {
          logInfo(
            "getResourceAttributes",
            `Schema extension ${schemaExtensionId} not yet applied to ${targetType}/${resourceId}`,
          );
          return {};
        }
  ...
  const updateResourceAttributes = React.useCallback(
    async (
      ...
      try {
        ...
        await client.api(`${endpoint}/${resourceId}`).patch(updatePayload);

        logInfo(
          "updateResourceAttributes",
          `Updated attributes for ${targetType}/${resourceId}`,
        );
