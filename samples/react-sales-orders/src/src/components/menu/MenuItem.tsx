...
import { Body1Strong, Caption1, mergeClasses, tokens } from "@fluentui/react-components";
    ...
    return (
      <div className={styles.itemContainer}>
        <div className={styles.itemTitle}>
          {icon}
          <Body1Strong className={styles.headerTitle} style={{ color: color }}>
            {title}
          </Body1Strong>
        </div>
        <Caption1 className={mergeClasses(styles.headerDescription, styles.itemDescription)}>{description}</Caption1>
      </div>
    );
  };
  return (
    <>
      <div style={{ color: color }} className={mergeClasses(styles.menuItem)} onClick={() => onItemClick()}>
        <ItemInfo
          ...
          icon={<Icon icon={`${icon}`} width={24} height={24} color={color} />}
        />
      </div>
    </>
  );
